#!/usr/bin/env bash
# Purpose: Onboard a GCP project’s Defender for Servers (MDE) resources to the organization-level Workload Identity Federation pool.
# =============================================================================
# Per-project onboarding to ORG-level pool for Defender for Servers
...
# Minimal roles for Servers features
...
# Allow org-level pool to impersonate this SA
gcloud iam service-accounts add-iam-policy-binding "${SERVERS_SA_EMAIL}"   --project "${PROJECT_ID}" --role="roles/iam.workloadIdentityUser"   --member="principalSet://iam.googleapis.com/organizations/${ORG_NUMBER}/locations/global/workloadIdentityPools/${POOL_ID}/*"

echo "Project '${PROJECT_ID}' is ready for Defender for Servers via org pool '${POOL_ID}'."
